...
import com.mansi.quizassignment.databinding.FragmentEssayBinding
...
class EssayFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val binding = DataBindingUtil.inflate<FragmentEssayBinding>(inflater, R.layout.fragment_essay,container,false)
        val safeArgs: EssayFragmentArgs by navArgs()
        val position = safeArgs.flowStepNumber
        val dataList = safeArgs.dataResponse

        binding.essayText.text= dataList?.data?.get(position)?.let { HtmlCompat.fromHtml(it.text, HtmlCompat.FROM_HTML_MODE_COMPACT) }
    ...
    private fun timer(binding: FragmentEssayBinding, dataList: ItemResponse?, flowStepNumber: Int) {
    ...
    private fun onClickListener(binding: FragmentEssayBinding, dataList: ItemResponse?, flowStepNumber: Int) {
